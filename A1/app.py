...
http://0.0.0.0:8000/api/v1/users/<nishi>


API 3
add ride post

http://0.0.0.0:8000/api/v1/rides
...
API  4 GET
 http://0.0.0.0:8000/api/v1/rides?source=10&destination=11

API 5 GET  given rideID


http://0.0.0.0:8000/api/v1/rides/1


API 6 JOIN RIDE POST
http://0.0.0.0:8000/api/v1/rides/<ride_query_ID>
...
def add_user():
	...
	r = requests.put('http://0.0.0.0:8000/api/v1/db/write', json=data)
	if(r.text=="0"):
...
def delete_user(username):
	...
	data["flag"]="2"
	data["username"]=username
	r = requests.delete('http://0.0.0.0:8000/api/v1/db/write', json=data)
	if(r.text=="0"):
...
def create_new_ride():
	...
	ride["flag"]="3"
	r = requests.post('http://0.0.0.0:8000/api/v1/db/write', json=ride)
	#return r.text
	if(r.text=="0"):
...
def rides():
	...
	query["time"]=act_time	
	r = requests.post('http://0.0.0.0:8000/api/v1/db/read',json=query)
	#return "hjj"	
	if(r.text=="0"):
...
def write_db():
	...
	if(catch["flag"]=="3"):
		...
		if(int(source) < 1 or int(destination) > 198):
			return "0"
		...
		hell=cur.execute("insert into Ride_table values(%s,%s,%s,%s,%s)",(ride_id,source,destination,creator,time))
	...
	if(catch["flag"]=="6"):
		...
		#return jsonify(int(ride))	
		row_c=row_c=cur.execute("select * from Ride_table where ride_id =%s",(int(ride),))
		#return jsonify(row_c)
		...
		no_count=cur.execute("select * from Riders_table where ride_id =%s AND rider =%s",(int(ride),str(username)))
		if(no_count>0):
			return "2"#abort(400,'{"message":"RIDER ALREADY IN GIVEN RIDE"}')
		
		hell=cur.execute("insert into Riders_table values(%s,%s)",(int(ride),str(username)))
		mysql.connection.commit()
...
def db_read():
	...
	if(query["flag"]=="5"):
		...
		if(len(super_dict["rideId"]) == 1):
			super_dict["rideId"]  = super_dict["rideId"][0]
